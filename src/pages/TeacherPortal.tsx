        ...
        <DashboardLayout>
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div className="lg:col-span-1 flex flex-col items-center">
                    <div className="w-[320px] h-[640px] bg-slate-900 rounded-[3rem] border-[10px] border-slate-800 p-4 relative shadow-2xl overflow-hidden shadow-blue-500/10">
                        {/* Notch */}
                        ...
                        <div className="bg-slate-50 w-full h-full rounded-2xl overflow-hidden flex flex-col relative z-10 font-sans">
                            ...
                            <div className="bg-blue-700 text-white p-6 pb-8 rounded-b-3xl shadow-sm z-10">
                                <div className="mt-4 flex justify-between items-center mb-6">
                                    <div>
                                        <p className="text-blue-200 text-xs font-medium uppercase tracking-wider">Behavioral Reports</p>
                                        <h2 className="font-bold text-xl">Good Morning, Mr. Cole</h2>
                                    </div>
                                    <div className="w-10 h-10 rounded-full bg-blue-500 border-2 border-blue-400"></div>
                                </div>

                                <div className="flex bg-blue-900/30 rounded-lg p-1 backdrop-blur-sm">
                                    <button
                                        onClick={() => setActiveTab('attendance')}
                                        className={`flex-1 py-1.5 text-sm font-medium rounded-md transition-colors ${activeTab === 'attendance' ? 'bg-white text-blue-800 shadow-sm' : 'text-blue-100 hover:text-white'}`}
                                    >
                                        Attendance
                                    </button>
                                    <button
                                        onClick={() => setActiveTab('behavior')}
                                        className={`flex-1 py-1.5 text-sm font-medium rounded-md transition-colors ${activeTab === 'behavior' ? 'bg-white text-blue-800 shadow-sm' : 'text-blue-100 hover:text-white'}`}
                                    >
                                        Report
                                    </button>
                                </div>
                                    ...
                                    <div className="space-y-4">
                                        ...
                                        <div className="flex rounded-md bg-slate-200/60 p-1">
                                            <button
                                                ...
                                                className={`flex-1 text-xs font-medium py-1.5 rounded ${attendanceMode === 'scan' ? 'bg-white shadow-sm text-blue-700' : 'text-slate-500'}`}
                                            >
                                                Scan QR
                                            </button>
                                            <button
                                                onClick={() => setAttendanceMode('manual')}
                                                className={`flex-1 text-xs font-medium py-1.5 rounded ${attendanceMode === 'manual' ? 'bg-white shadow-sm text-blue-700' : 'text-slate-500'}`}
                                            >
                                                Manual
                                            </button>
                                        </div>
                                        ...
                                        {attendanceMode === 'scan' ? (
                                            <div className="bg-white p-4 rounded-xl shadow-sm border border-slate-100 flex flex-col items-center justify-center py-10 gap-4 mt-2">
                                                <div className="w-20 h-20 bg-blue-50 rounded-full flex items-center justify-center text-blue-600 cursor-pointer hover:bg-blue-100 transition-colors hover:scale-105 active:scale-95">
                                                ...
                                                {students.map(student => (
                                                    <div key={student.id} className="bg-white rounded-xl border border-slate-200 p-3 shadow-sm flex flex-col gap-2 relative">
                                                        ...
                                                        {student.status === 'L' && student.comment && (
                                                            <div className="bg-blue-50 px-2 py-1 text-xs text-blue-700 rounded flex gap-1 items-center">
                                    ...
                                    <div className="space-y-4">
                                        <div className="flex rounded-md bg-slate-200/60 p-1">
                                            <button
                                                ...
                                                className={`flex-1 text-xs font-medium py-1.5 rounded ${reportInputMode === 'search' ? 'bg-white shadow-sm text-blue-700' : 'text-slate-500'}`}
                                            >
                                                Search Name
                                            </button>
                                            <button
                                                onClick={() => setReportInputMode('scan')}
                                                className={`flex-1 text-xs font-medium py-1.5 rounded ${reportInputMode === 'scan' ? 'bg-white shadow-sm text-blue-700' : 'text-slate-500'}`}
                                            >
                                                Scan QR
                                            </button>
                                        </div>
                                        ...
                                        <div className="bg-white p-4 rounded-xl shadow-sm border border-slate-100 space-y-4 mt-2">
                                            {reportInputMode === 'scan' ? (
                                                <div className="flex flex-col items-center justify-center py-6 gap-3">
                                                    <div className="w-16 h-16 bg-blue-50 rounded-full flex items-center justify-center text-blue-600 cursor-pointer hover:bg-blue-100 transition-colors hover:scale-105 active:scale-95">
                                                ...
                                                <div>
                                                    ...
                                                    <div className="relative">
                                                        ...
                                                        <input type="text" className="w-full bg-slate-50 border border-slate-200 rounded-lg pl-9 pr-3 py-2 text-sm focus:ring-1 focus:ring-blue-500 focus:outline-none" placeholder="Search..." />
                                                    </div>
                                                </div>
                                            )}
                                            ...
                                            <div>
                                                ...
                                                <select className="w-full bg-slate-50 border border-slate-200 rounded-lg px-3 py-2 text-sm focus:ring-1 focus:ring-blue-500 focus:outline-none text-slate-700">
                                            ...
                                            <button className="w-full py-2.5 bg-white hover:bg-blue-600 text-white rounded-lg text-sm font-semibold flex items-center justify-center gap-2 transition-colors">
                            ...
                            <div className="space-y-4">
                                <div>
                                    ...
                                    <div className="relative">
                                        ...
                                        <input type="text" className="w-full bg-slate-50 border border-slate-200 rounded-xl pl-10 pr-4 py-3 text-sm focus:ring-2 focus:ring-blue-500 focus:outline-none transition-all placeholder:text-slate-400" placeholder="e.g. Alex Johnson or 900-1204" />
                                    </div>
                                </div>
                                <div className="space-y-3 relative">
                                    <label className="font-semibold text-slate-800 text-sm flex items-center justify-between">
                                        <span>Raw Teacher Input</span>
                                        <button className="text-blue-600 hover:bg-blue-50 p-1.5 rounded-full transition-colors" title="Dictate">
                                            <Mic className="w-4 h-4" />
                                        </button>
                                    </label>
                                    <textarea
                                        className="w-full h-32 bg-slate-50 border border-slate-200 rounded-xl p-4 text-sm focus:ring-2 focus:ring-blue-500 focus:border-transparent resize-none text-slate-700 leading-relaxed placeholder:text-slate-400"
